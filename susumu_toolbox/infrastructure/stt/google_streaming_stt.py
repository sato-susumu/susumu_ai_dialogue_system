...
from susumu_toolbox.infrastructure.stt.base_stt import BaseSTT, STTResult, MicrophoneStream
...
class GoogleStreamingSTT(BaseSTT):
    ...
    def _on_timeout(self):
        ...
        if self._stream:
            self._stream.close()
            self._stream = None
        self._event_channel.publish(self.EVENT_STT_RESULT, STTResult(self._transcript, True, True))
    ...
    def _processing(self, response_generator):
        ...
        try:
            for response in response_generator:
                ...
                self._event_channel.publish(self.EVENT_STT_DEBUG_MESSAGE, response)
                ...
                if result.is_final:
                    self._event_channel.publish(self.EVENT_STT_RESULT, STTResult(self._transcript, True))
                    if self._single_utterance:
                        break
                else:
                    # is_final=Falseで返ってきた結果は認識精度が低いため、表示以外の用途で使うのはオススメできない。
                    # 不鮮明な音だと is_final=False で何度か結果が返ってくるものの、その後何も反応がないことがある。
                    self._event_channel.publish(self.EVENT_STT_RESULT, STTResult(self._transcript, False))
        finally:
            if timer:
                timer.cancel()
