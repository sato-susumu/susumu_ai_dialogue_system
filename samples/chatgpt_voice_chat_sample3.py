...
class ChatGPTVoiceChatSample3(BaseVoiceChatSample):
    """ChatGPTボイスチャットのサンプル

    入力：音声認識(GoogleStreamingSTT)
    応答生成：ChatGPT
    応答生成前後の翻訳：なし
    出力：画面出力、音声合成(Google音声合成)
    """
    ...
    def create_chat(self) -> BaseChat:
        ...
        path = os.path.join(system.get_config_dir(), "sample_system_settings2.txt")
    ...
    def create_tts(self) -> BaseTTS:
        return GoogleCloudTTS(self._config)
...
if __name__ == "__main__":
    _config = Config()
    _config.load()
    ChatGPTVoiceChatSample3(_config).run_forever()
